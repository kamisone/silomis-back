...
export class CartService {
  constructor(
    ...
    @InjectQueue(CART_ABANDONMENT_QUEUE) private readonly abandonmentQueue: Queue<CartAbandonmentJobData>,
  ) {}
  ...
  async getOrCreate(token: string, userId?: string, lang?: string) {
    const existing = await this.prisma.cart.findUnique({ where: { token }, include: { items: true } });

    if (existing && existing.status === 'active') {
      if (userId && !existing.userId) {
        await this.prisma.cart.update({ where: { id: existing.id }, data: { userId } });
  ...
  async addItem(token: string, variantId: string, quantity: number, selectedOptionValueIds?: string[], lang?: string, meta?: RequestMeta) {
    if (quantity < 1) throw new BadRequestException('Quantity must be at least 1');

    const variant = await this.prisma.productVariant.findUnique({
      where: { id: variantId },
      include: { product: true, options: { include: { attribute: true, optionValue: true } } },
    });
    ...
    if (existing) {
      ...
      const unitPriceCents = resolveUnitPriceForQuantity({ variantPriceCents: variant.priceCents, basePriceCents: product.basePriceCents, optionAdjustmentCents: sumOptionAdjustments(variant.options) }, newQty, { upsellingEnabled: product.upsellingEnabled, upsellTiers: product.upsellTiers as never });
      await this.prisma.cartItem.update({ where: { id: existing.id }, data: { quantity: newQty, unitPriceCents } });
    } else {
      ...
      if (selectedOptionValueIds?.length) {
        const ovRows = await this.prisma.variationOptionValue.findMany({ where: { id: { in: selectedOptionValueIds } }, include: { attribute: true } });
      ...
      const unitPriceCents = resolveUnitPriceForQuantity({ variantPriceCents: variant.priceCents, basePriceCents: product.basePriceCents, optionAdjustmentCents: sumOptionAdjustments(variant.options) }, quantity, { upsellingEnabled: product.upsellingEnabled, upsellTiers: product.upsellTiers as never });
      ...
      if (!imageKeySnapshot && optionsSnapshot?.length) {
        ...
        if (optionValueIds.length) {
          const optionImage = await this.prisma.productOptionValueImage.findFirst({ where: { productId: product.id, optionValueId: { in: optionValueIds } } });
    ...
    await this.abandonmentQueue.add('abandon', { cartToken: token }, { delay: ABANDONMENT_DELAY_MS, jobId: `cart-abandon.${token}`, removeOnComplete: true });

    await this.behaviorTracking.record({ eventType: 'add_to_cart', cartToken: token, productId: product.id, quantity, clientIp: meta?.ip, userAgent: meta?.userAgent });
  ...
  async updateItem(token: string, itemId: string, quantity: number, lang?: string, meta?: RequestMeta) {
    ...
    const inventory = await this.prisma.inventoryItem.findUnique({ where: { variantId: item.variantId } });
    if (!inventory || inventory.available < quantity) {
      throw new BadRequestException({ code: 'INSUFFICIENT_STOCK', available: inventory?.available ?? 0 });
    }
    ...
    const [variant, product] = await Promise.all([this.prisma.productVariant.findUnique({ where: { id: item.variantId }, include: { options: { include: { optionValue: true } } } }), this.prisma.product.findUnique({ where: { id: item.productId } })]);
    if (!variant || !product) {
      throw new BadRequestException('This item is no longer available. Please remove it from your cart.');
    }

    const unitPriceCents = resolveUnitPriceForQuantity({ variantPriceCents: variant.priceCents, basePriceCents: product.basePriceCents, optionAdjustmentCents: sumOptionAdjustments(variant.options) }, quantity, { upsellingEnabled: product.upsellingEnabled, upsellTiers: product.upsellTiers as never });
    await this.prisma.cartItem.update({ where: { id: item.id }, data: { quantity, unitPriceCents } });
    await this.behaviorTracking.record({ eventType: 'update_cart_item', cartToken: token, productId: item.productId, quantity, clientIp: meta?.ip, userAgent: meta?.userAgent });
    return this.getOrCreate(token, undefined, lang);
  }

  // ── Remove item ──────────────────────────────────────────────────────

  async removeItem(token: string, itemId: string, lang?: string, meta?: RequestMeta) {
    const cart = await this.ensureActiveCart(token);
    const item = cart.items.find((i) => i.id === itemId);
    if (!item) throw new NotFoundException('Cart item not found');
    await this.prisma.cartItem.delete({ where: { id: item.id } });
    await this.behaviorTracking.record({ eventType: 'remove_from_cart', cartToken: token, productId: item.productId, quantity: item.quantity, clientIp: meta?.ip, userAgent: meta?.userAgent });
    return this.getOrCreate(token, undefined, lang);
  }
  ...
  async markCompleted(token: string): Promise<void> {
    await this.prisma.cart.updateMany({ where: { token }, data: { status: 'completed' } });
  }

  // ── Internal helpers ──────────────────────────────────────────────────

  private async ensureActiveCart(token: string): Promise<Cart & { items: CartItem[] }> {
    const cart = await this.prisma.cart.findFirst({ where: { token, status: 'active' }, include: { items: true } });
  ...
  private async getOrCreatePersistedCart(token: string): Promise<Cart & { items: CartItem[] }> {
    const existing = await this.prisma.cart.findUnique({ where: { token }, include: { items: true } });
    ...
    if (existing) {
      ...
      if (existing.status !== 'active') throw new NotFoundException('Active cart not found');
      return existing;
    }

    const cart = await this.prisma.cart.create({ data: { token, status: 'active' } });
    return { ...cart, items: [] };
  }
  ...
  private async enrichCart(cart: Cart & { items: CartItem[] }, lang?: string) {
    ...
    if (lang) {
      const snapshots = enrichedItems.map((i) => (i.optionsSnapshot as unknown as OptionSnapshot[] | null) ?? []);
      const uniqueAttrIds = [...new Set(snapshots.flatMap((s) => s.map((o) => o.attributeId).filter(Boolean)))];
      const uniqueOptionIds = [...new Set(snapshots.flatMap((s) => s.map((o) => o.optionValueId).filter(Boolean) as string[]))];
      ...
      const attrMap = new Map(attrTranslated.map((r: Record<string, unknown>) => [r.id, r]));
      const optionMap = new Map(optionTranslated.map((r: Record<string, unknown>) => [r.id, r]));
      const titleMap = new Map(productTitleTranslated.map((r: Record<string, unknown>) => [r.id, r.title]));

      enrichedItems = enrichedItems.map((item) => ({
        ...item,
        titleSnapshot: (titleMap.get(item.productId) as string | undefined) ?? item.titleSnapshot,
        optionsSnapshot: ((item.optionsSnapshot as unknown as OptionSnapshot[] | null) ?? []).map((o) => ({
          ...o,
          attributeName: (attrMap.get(o.attributeId) as Record<string, unknown> | undefined)?.name ?? o.attributeName,
          displayValue: o.optionValueId ? ((optionMap.get(o.optionValueId) as Record<string, unknown> | undefined)?.displayValue ?? o.displayValue) : o.displayValue,
        })) as never,
      }));
    }
    ...
    return {
      ...
      freeShipping: enrichedItems.length > 0 && enrichedItems.every((i) => i.freeShipping === true),
    };
  }
}
