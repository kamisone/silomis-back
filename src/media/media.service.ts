...
import { MediaAsset, MediaFolder, MediaUsage } from '../../generated/prisma/client';
...
function deriveFilenameFromUrl(sourceUrl: string, contentType: string): string {
  const lastSegment = decodeURIComponent(sourceUrl.split('/').pop()?.split('?')[0] || '');
  const looksLikeRealFilename =
    lastSegment.length > 0 && lastSegment.length <= 200 && /\.[a-z0-9]{2,5}$/i.test(lastSegment);
...
export class MediaService {
  ...
  async upload(
    ...
    return this.persistBuffer(file.buffer, file.originalname, file.mimetype, altText, uploadedBy, folderId);
  }
  ...
  private async persistBuffer(
    ...
    const existing = await this.prisma.mediaAsset.findFirst({ where: { checksum } });
    if (existing) {
    ...
    await this.gcs.upload(buffer, storageKey, mimeType, 'publicRead', MEDIA_CACHE_CONTROL);

    const { width, height, durationSeconds } =
      kind === 'video'
        ? await extractVideoMetadata(buffer, mimeType)
        : { ...(await extractImageDimensions(buffer, mimeType)), durationSeconds: null };
  ...
  async ingestFromUrl(
    sourceUrl: string,
    opts: { altText?: string; uploadedBy?: string; folderId?: string | null } = {},
  ): Promise<(MediaAsset & { url: string; mediaType: MediaKind }) | null> {
    const shortUrl = sourceUrl.length > 150 ? `${sourceUrl.slice(0, 150)}…` : sourceUrl;
    try {
      const res = await fetch(sourceUrl, { signal: AbortSignal.timeout(15_000) });
      if (!res.ok) {
        this.logger.warn(`ingestFromUrl got HTTP ${res.status} for ${shortUrl}`);
        return null;
      }
      const contentType = res.headers.get('content-type')?.split(';')[0]?.trim() ?? '';
      if (!ALLOWED_IMAGE_MIME_TYPES.includes(contentType)) {
        this.logger.warn(`ingestFromUrl got unsupported content-type "${contentType}" for ${shortUrl}`);
        return null;
      }
      const arrayBuf = await res.arrayBuffer();
      if (arrayBuf.byteLength === 0 || arrayBuf.byteLength > MAX_IMAGE_BYTES) {
        this.logger.warn(`ingestFromUrl got ${arrayBuf.byteLength} bytes (outside allowed range) for ${shortUrl}`);
        return null;
      }
      const buffer = Buffer.from(arrayBuf);
      const filename = deriveFilenameFromUrl(sourceUrl, contentType);
      return await this.persistBuffer(buffer, filename, contentType, opts.altText, opts.uploadedBy, opts.folderId);
    } catch (err) {
      this.logger.warn(`ingestFromUrl failed for ${shortUrl}: ${(err as Error).message}`);
      return null;
    }
  }
  ...
  async list(opts: MediaListOptions = {}): Promise<{
    items: Array<MediaAsset & { url: string; usageCount: number; mediaType: MediaKind }>;
    total: number;
  }> {
    const { search, mimeType, mediaType, tag, folderId, folderSet, limit = 48, offset = 0 } = opts;

    const where = {
      ...(search ? { originalFilename: { contains: search, mode: 'insensitive' as const } } : {}),
      ...(mimeType ? { mimeType } : {}),
      ...(mediaType ? { mimeType: { startsWith: `${mediaType}/` } } : {}),
      ...(tag ? { tags: { has: tag } } : {}),
      ...(folderSet ? { folderId: folderId === null || folderId === '' ? null : folderId } : {}),
    };

    const [assets, total] = await Promise.all([
      this.prisma.mediaAsset.findMany({ where, orderBy: { createdAt: 'desc' }, take: limit, skip: offset }),
    ...
    const usageCounts = ids.length
      ? await this.prisma.mediaUsage.groupBy({ by: ['assetId'], where: { assetId: { in: ids } }, _count: { id: true } })
      : [];
  ...
  async findByStorageKeys(keys: string[]): Promise<MediaAsset[]> {
    ...
    return this.prisma.mediaAsset.findMany({ where: { storageKey: { in: [...new Set(keys)] } } });
  }
  ...
  async moveAssets(assetIds: string[], folderId: string | null): Promise<void> {
    ...
    await this.prisma.mediaAsset.updateMany({ where: { id: { in: assetIds } }, data: { folderId } });
  }
  ...
  async delete(id: string): Promise<void> {
    ...
    const usageCount = await this.prisma.mediaUsage.count({ where: { assetId: id } });
    if (usageCount > 0) {
  ...
  async listFolders(): Promise<Array<MediaFolder & { assetCount: number; thumbnailUrl: string | null }>> {
    const folders = await this.prisma.mediaFolder.findMany({ orderBy: { name: 'asc' } });
    ...
    const countMap = new Map(counts.map((r) => [r.folderId as string, r._count.id]));
    ...
    const thumbCandidates = await this.prisma.mediaAsset.findMany({
      where: {
        folderId: { not: null },
        OR: [{ mimeType: { startsWith: 'image/' } }, { autoPosterKey: { not: null } }],
      },
      orderBy: { createdAt: 'desc' },
      select: { folderId: true, mimeType: true, storageKey: true, autoPosterKey: true },
    });
    ...
    for (const c of thumbCandidates) {
      ...
      const key = c.mimeType.startsWith('image/') ? c.storageKey : c.autoPosterKey;
  ...
  async createFolder(name: string, parentId?: string | null): Promise<MediaFolder & { assetCount: number; thumbnailUrl: string | null }> {
    if (parentId) {
      const parent = await this.prisma.mediaFolder.findUnique({ where: { id: parentId } });
      if (!parent) throw new NotFoundException('Parent folder not found');
    }
    const folder = await this.prisma.mediaFolder.create({ data: { name: name.trim(), parentId: parentId ?? null } });
    return { ...folder, assetCount: 0, thumbnailUrl: null };
  }

  async renameFolder(id: string, name: string): Promise<MediaFolder & { assetCount: number; thumbnailUrl: string | null }> {
    const existing = await this.prisma.mediaFolder.findUnique({ where: { id } });
    if (!existing) throw new NotFoundException('Folder not found');
    await this.prisma.mediaFolder.update({ where: { id }, data: { name: name.trim() } });
  ...
  async deleteFolder(id: string): Promise<void> {
    ...
    await this.prisma.mediaAsset.updateMany({ where: { folderId: id }, data: { folderId: folder.parentId } });
    await this.prisma.mediaFolder.updateMany({ where: { parentId: id }, data: { parentId: folder.parentId } });
  ...
  async removeUsage(assetId: string, entityType: string, entityId: string, field: string): Promise<void> {
    await this.prisma.mediaUsage.deleteMany({ where: { assetId, entityType, entityId, field } });
  }

  async getUsage(assetId: string): Promise<MediaUsage[]> {
    return this.prisma.mediaUsage.findMany({ where: { assetId }, orderBy: { createdAt: 'desc' } });
  }

  async syncEntityUsages(entityType: string, entityId: string, storageKeys: Array<{ key: string; field: string }>): Promise<void> {
    await this.prisma.mediaUsage.deleteMany({ where: { entityType, entityId } });
    if (!storageKeys.length) return;

    const uniqueKeys = [...new Set(storageKeys.map((s) => s.key))];
    const assets = await this.prisma.mediaAsset.findMany({ where: { storageKey: { in: uniqueKeys } } });
    const keyToId = new Map(assets.map((a) => [a.storageKey, a.id]));

    const records = storageKeys
      .filter(({ key }) => keyToId.has(key))
      .map(({ key, field }) => ({ assetId: keyToId.get(key)!, entityType, entityId, field }));

    if (records.length) await this.prisma.mediaUsage.createMany({ data: records });
  }
}
